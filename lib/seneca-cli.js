...
vorpal.command('set-config', 'set the config')
    .option('-u', '--url <url>')
    .option('-p', '--pin <pin>')
    .action(function setConfig(args, callback) {
        if (args.pin) {
            options.pin = args.pin;
            this.log(`set pin: ${options.pin}`);
        }
        if (args.url) {
            options.url = args.url;
            this.log(`set url: ${options.url}`);
        }
        callback('set config successful');
    });

vorpal.command('get-config', 'set the config')
    .action(function setConfig(args, callback) {
        if (options.pin) {
            this.log(`pin: ${options.pin}`);
        }
        if (options.url) {
            this.log(`url: ${options.url}`);
        }
        callback('get config successful');
    });
