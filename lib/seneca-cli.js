        ...
        const fnFactory = function (client, run) {
            ...
            return function fn() {
                ...
                return new Promise((resolve, reject) => {
                    _v.log(`Starting run in promise: ${run.name}`);
        ...
        seneca.clientAsync(options)
            .then((senecaClient) => {
                ...
                for (let i = 0; i < loadRunnerConfig.loop; i += 1) {
                    ...
                    perLoopFnArray.concat(innerFnArray);
                }
                _v.log(`complete setting up perLoopFn array: ${perLoopFnArray.length}`);
                _v.log(`assert type: ${typeof perLoopFnArray[0] === 'function'}`);
                _v.log(`assert type: ${typeof perLoopFnArray[0]}`);
